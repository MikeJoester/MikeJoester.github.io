...
const CreatePostButton = () => {
  ...
  const headerData = {
   ...
   const cateList = cate.map((i)=> <option value={i.id} className="rounded-none">{i.name}</option>);
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-slate-800 outline-none focus:outline-none">
                    <div className="flex p-5 border-b border-solid border-slate-200 rounded-t justify-between">
                        ...
                        <button className="px-3 pb-2 bg-transparent border-0 text-white float-right leading-none font-semibold outline-white hover:bg-slate-200 hover:text-slate-800 rounded-xl" onClick={() => setShowPostModal(false)}>
                    ...
                    <div className="flex flex-col p-6 space-y-6">
                        <div className="flex flex-row space-x-4">
                            <label for="categories" className="text-left font-light w-1/3">Choose Category:</label>
                            <select 
                                id="categories"
                                onChange={(e)=>setId(e.target.value)}
                                value={idNum}
                                className="w-2/3 rounded-none text-black font-normal"
                            >
