...
const EditCategory = (props) => {
  ...
  const onClickSave = async(e) => {
    ...
    try {
      await axios.patch(`https://dev.thanqminh.com:3001/task_lists/${props.info.id}`, props.info, headerData);
  ...
  return (
    <>
      <button onClick={() => setEdit(true)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
                <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-slate-800 outline-none focus:outline-none">
                    <div className="flex p-5 border-b border-solid border-slate-200 rounded-t justify-between">
                        <h3 className="bg-transparent text-3xl block font-semibold text-white">
                            Edit Category
                        </h3>
                    ...
                    <div className="flex flex-col p-6 space-y-6">
                        ...
                        <div className="flex flex-row justify-between">
                            ...
                            <button className="flex-1 w-1/3 bg-slate-500 px-5 py-2 text-xl rounded-lg hover:bg-slate-400 self-right text-white" onClick={onClickSave}>Save</button>
                        </div>
                    </div>
                </div>
            </div>
